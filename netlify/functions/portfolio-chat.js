// Netlify serverless function — source: scripts/portfolio-chat.js
...
const CSE_API_KEY    = process.env.CSE_API_KEY || "";            // optional
...
// Avoid cutting answers mid-sentence. Keep this below Netlify response limits.
const MAX_OUT_CHARS = 4200;
const MAX_LINK_CHARS = 14000; // max extracted JD text passed to model

// Optional: load KB server-side so the widget isn't the only source of truth.
let KB_FALLBACK = null;
try {
  // The function bundle runs with repo files available.
  // If this fails (bundler/execution differences), we simply rely on client-provided kb.
  const fs = require("fs");
  const path = require("path");
  const kbPath = path.join(__dirname, "..", "..", "assets", "portfolio-kb.json");
...
function polishAnswerText(text = "") {
  ...
  // Replace dash variants with normal punctuation.
  s = s.replace(/—/g, ", ");
  s = s.replace(/–/g, "-");

  // Remove common banned/AI-ish phrases if the model slips.
  ...
  // Clean up accidental double spaces/punctuation.
  s = s.replace(/\s{2,}/g, " ");
  s = s.replace(/\s+,/g, ",");
  s = s.replace(/,\s*,/g, ",");
  return s.trim();
...
function isPrivateHostname(hostname = "") {
  ...
  // Block common private network patterns when users paste internal links.
...
function allowedExternalHost(urlObj) {
  const hosts = (process.env.ALLOWED_LINK_HOSTS || "")
    ...
    .filter(Boolean);
  if (!hosts.length) return true; // allow any public host by default
...
function stripHtmlToText(html = "") {
  ...
  // Convert breaks/paragraph-ish tags to newlines
...
function compactExcerpt(text = "", maxChars = MAX_LINK_CHARS) {
  // Normalize horizontal spaces but preserve paragraph newlines
  let s = String(text || "")
    .replace(/[ \t]+/g, " ")
    .replace(/\r/g, "")
    .replace(/\n{3,}/g, "\n\n")
...
async function fetchReadableTextViaJina(urlObj) {
  // Fallback for JS-heavy pages where HTML fetch returns a shell.
  // r.jina.ai returns a readable text extraction for public pages.
...
function buildRouterHint(kb) {
  ...
  // Flatten keyword map into "keyword → Case Title (URL)" lines when possible
  const lines = [];
  for (const [keyword, caseIdOrSlug] of Object.entries(map)) {
    ...
    if (!first) continue;
    // Try to resolve case by id/slug/title match
    const c =
      (kb.cases || []).find(x =>
...
function detectFitIntent(text = "") {
  ...
  // Medium signals (often fit, but not always)
  const medium =
...
function extractCompanyHint(text = "") {
  ...
  for (const re of patterns) {
    ...
    if (m && m[1]) {
      ...
      // Trim trailing question word fragments like "?" leftovers
...
function extractRoleHint(text = "") {
  ...
  // "fit for Senior Product Designer at Notion"
  const m =
...
// Selective hire intent evaluation - only show LinkedIn CTA for genuine high-intent moments
...
// Deterministic fallbacks for simple, high-value intents
function getDeterministicAnswer(intent, kb) {
  if (!kb) return null;

  // Prefer hand-curated deterministic answers from the KB if present.
  const curated = kb.deterministic_answers || {};
  if (typeof curated[intent] === 'string' && curated[intent].trim()) {
    return curated[intent].trim();
  }

  // Aliases: map detected intents to KB keys.
  const intentAliases = {
  ...
  // Fall back to constructed answers from structured KB fields.
...
function chooseCaseIdsFromText(kb, messagesArray, fallbackText, max = 2, pageUrl = "", intent = "general", isFit = false) {
  ...
  // Only include the page case when the question is case-relevant or fit-related.
...
function buildMetricAllowlist(kb) {
  ...
  // Extract percentages, decimals, counts with +, and simple numbers
  const re = /\b\d[\d,\.]*\s*(?:%|\+)?/g;
  let m;
...
function pickRelevantExamples(kb, intent, lastUserMsg = "", max = 4) {
  ...
  const intentAliases = {
    ...
    resume: ["resume_proof"],
    // ai_adept_fit gets triggered through role_fit + AI keywords
  ...
  // Backup: if intent matching found too little, look for keyword overlap.
  if (picked.length < 2) {
    const words = t
      .split(/\s+/)
  ...
  // Always include one baseline style example if we didn't get enough.
  if (picked.length < 2) {
...
function repairFitAnswer(answer, isFit, kb) {
  ...
  // Ensure bullets end with periods
  s = s.replace(/^(\s*[-*•]\s+.+)(?<![.!?])$/gm, "$1.");

  // Cap bullets at 4 for fit answers
  const lines = s.split('\n');
...
function parseGeminiOutput(text, pickedCases, kb, isFit = false) {
  ...
  try {
    ...
    const rawCases = Array.isArray(parsed.context_cases) ? parsed.context_cases : [];
...
function buildEvidenceByRoleContext(kb, lastUserMsg = "") {
  ...
  // Map user signals to role keys in the KB
  const roleMatchers = [
...
function mergeConsecutiveTurns(contents) {
  const merged = [];
...
async function callGemini(systemPrompt, messages, webSnippets, kb, answerStyle, options = {}) {
  ...
  // Make format/structure constraints explicit and consistent.
...
if (examples.length) {
  ...
  // ✅ ROUTER CONTEXT (new): helps the model select the right case deterministically
...
// New KB sections: looking_for, engineering, points_of_view, meta_qa
...
// Evidence by role — only when a fit/role question is in play
...
// Company fit patterns — only when a fit question is in play
...
// Negative rules — always include when KB has them
...
if (negativeCtx && !simpleIntent) {
  ...
  // Focused case context: pick likely-relevant cases and pass richer details.
  let pickedCases = [];
  if (kb?.cases?.length) {
...
// Only add COMPANY/ROLE hints for fit questions (prevents polluting normal Q&A)
if (isFit) {
  const companyHint = cleanHint(extractCompanyHint(lastUserMsg));
  const roleHint = cleanHint(extractRoleHint(lastUserMsg));

  // Add the hints BEFORE the TASK MODE instruction so the model anchors first
  ...
  // (Optional but recommended) Reinforce fit-mode explicitly
  contents.push({
    role: "user",
    parts: [{
      text:
...
`
  ...
  const body = {
    ...
    contents: mergeConsecutiveTurns(contents),
    generationConfig: {
      temperature,
      topP: 0.9,
      topK: 32,
...
async function logIfEnabled(payload) {
  ...
  try {
    ...
    if (LOG_WEBHOOK_URL.includes("discord.com/api/webhooks")) {
      ...
      bodyObj = {
        embeds: [{
          title: isBlock ? "⚠️ Blocked Chat Query" : "💬 New Chat Query",
...
exports.handler = async (event) => {
  ...
  if (!GEMINI_API_KEY || GEMINI_API_KEY.trim() === "") {
    return json(assistantPayload({
      answer: "Chat is not configured. Add GEMINI_API_KEY in Netlify → Site settings → Environment variables."
    }), 500);
  }
  let lastUser = "";
  let intent = "general";
  try {
    ...
    // Use deterministic answer only for simple, high-value direct questions.
// Fit, comparison, JD, and nuanced prompts should go to Gemini.
...
return json(assistantPayload({
  ...
  hire_intent: selectiveHireIntent
    ...
    // If user pasted a JD link or raw job description text in a fit question, provide it to the model.
    let externalJd = null;
    if (isFit) {
    ...
    // Web search is an extra network call: only do it for fit/company-context questions.
    ...
    // Provide page context as a lightweight nudge (helps when user asks "this page" / "this case").
    ...
    const geminiOutput = await callGemini(SYSTEM_PROMPT, augmentedMessages, webSnippets, kb, answerStyle, {
    ...
    const answer = polishAnswerText(validateAnswerMetrics(geminiOutput.answer, metricAllowlist));
    const suggested_pills = Array.isArray(geminiOutput.suggested_pills) && geminiOutput.suggested_pills.length
      ? geminiOutput.suggested_pills.slice(0, 2)
      : defaultSuggestedPills(intent, lastUser);
    const messageCount = messages.filter(m => m.role === "user").length;
    const selectiveHireIntent = shouldShowHireIntent(intent, lastUser, messageCount, isFit);
    const hire_intent = geminiOutput.hire_intent && selectiveHireIntent;
    const sources = Array.isArray(geminiOutput.sources) ? geminiOutput.sources : [];
    const context_cases = Array.isArray(geminiOutput.context_cases) ? geminiOutput.context_cases.slice(0, 2) : [];
    const action_scroll_to = String(geminiOutput.action_scroll_to || "");
    const action_highlight = String(geminiOutput.action_highlight || "");
